...
public sealed class ApiExceptionHandler(ILogger<ApiExceptionHandler> logger) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext context, Exception exception, CancellationToken cancellationToken)
    {
        var (status, title) = exception switch
        {
            ...
            GuardianStudentLinksExistException => (StatusCodes.Status409Conflict, "Guardian has linked students"),
        ...
        await context.Response.WriteAsJsonAsync(new ProblemDetails { Status = status, Title = title, Detail = exception is TeachingAssignmentConflictException or StaffImportValidationException or GuardianStudentLinksExistException ? exception.Message : null, Instance = context.Request.Path, Extensions = { ["correlationId"] = context.Response.Headers["X-Correlation-ID"].ToString() } }, cancellationToken);
        return true;
    }
}
