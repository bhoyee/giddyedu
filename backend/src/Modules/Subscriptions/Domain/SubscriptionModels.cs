...
public sealed class TenantSubscription : ITenantOwned
{
    ...
    public void ChangePlan(Guid planId, DateTimeOffset now) { if (planId == Guid.Empty) throw new ArgumentException("Plan identifier is required.", nameof(planId)); if (Status == SubscriptionStatus.Cancelled) throw new InvalidOperationException("A cancelled subscription cannot change plan."); PlanId = planId; UpdatedAtUtc = now; }
