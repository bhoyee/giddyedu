...
{
    private Student() { }
    public Student(Guid id, Guid tenantId, string admissionNumber, string firstName, string lastName, DateOnly dateOfBirth, Guid? sourceApplicantId, DateTimeOffset now)
    { if (id == Guid.Empty || tenantId == Guid.Empty) throw new ArgumentException("Student identifiers are required."); Id = id; TenantId = tenantId; AdmissionNumber = Required(admissionNumber, 50).ToUpperInvariant(); FirstName = Required(firstName, 100); LastName = Required(lastName, 100); DateOfBirth = dateOfBirth; SourceApplicantId = sourceApplicantId; Status = StudentStatus.Active; CreatedAtUtc = now; }
    public Guid Id { get; private set; } public Guid TenantId { get; private set; } public string AdmissionNumber { get; private set; } = null!; public string FirstName { get; private set; } = null!; public string LastName { get; private set; } = null!; public DateOnly DateOfBirth { get; private set; } public Guid? SourceApplicantId { get; private set; } public StudentStatus Status { get; private set; } public DateTimeOffset CreatedAtUtc { get; private set; }
    public void UpdatePersonalInformation(string firstName, string lastName, DateOnly dateOfBirth) { FirstName = Required(firstName, 100); LastName = Required(lastName, 100); DateOfBirth = dateOfBirth; }
