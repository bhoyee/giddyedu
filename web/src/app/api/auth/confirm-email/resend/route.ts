...
export async function POST(request: NextRequest) {
  ...
  const upstream = await fetch(apiUrl("/api/v1/auth/confirm-email/resend"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(await request.json()), cache: "no-store" });
