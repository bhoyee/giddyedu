...
export async function POST(request: NextRequest) {
  ...
  const upstream = await fetch(apiUrl("/api/v1/auth/reset-password/code"), { method: "POST", headers: { "Content-Type": "application/json" }, body: await request.text(), cache: "no-store" });
