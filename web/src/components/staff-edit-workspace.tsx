...
export function StaffEditWorkspace({ staffId }: { staffId: string }) {
  ...
  return <main className="w-full space-y-6 pb-8">
    ...
    <form onSubmit={save} className="space-y-6">
      ...
      <div className="flex flex-wrap justify-end gap-3"><Link href={`/portal/staff/${staffId}`} className="rounded-xl border border-red-200 px-5 py-2.5 text-sm font-bold text-red-700 hover:bg-red-50">Cancel</Link><button disabled={saving} className="tenant-primary-bg rounded-xl px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{saving ? "Saving…" : "Save staff record"}</button></div>
    </form>
