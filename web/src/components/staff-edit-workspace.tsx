  ...
  return <main className="w-full space-y-6 pb-8">
    ...
    <form onSubmit={save} className="space-y-6">
      ...
      <Section title="Medical information" note="Medical details remain protected by the staff-sensitive permission."><Grid>
        ...
        <Field name="weightKg" label="Weight (kg)" type="number" step="any" value={privateValue("weightKg")} />
        <Field name="heightCm" label="Height (cm)" type="number" step="any" value={privateValue("heightCm")} />
...
function Field({ name, label, value, type = "text", required, readOnly, pattern, maxLength, step }: { name: string; label: string; value: string; type?: string; required?: boolean; readOnly?: boolean; pattern?: string; maxLength?: number; step?: string }) { return <label className="text-xs font-bold text-slate-700">{label}<input name={name} type={type} step={step} defaultValue={value} required={required} readOnly={readOnly} pattern={pattern} maxLength={maxLength} className={`${fieldClass} ${readOnly ? "bg-slate-100" : ""}`} /></label>; }
