...
export function StaffRegistrationWizard({ onCreated }: { onCreated?: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      notify({title:"Staff record saved successfully",message:`A welcome email has been queued for ${email}. Open the staff profile to add other positions and teaching duties.`}); setOpen(false); setStep(0); setDraft({}); setCategory(""); setState(""); setReligion(""); setQualification(false); setQualificationName(""); setGrade(""); setAddNextOfKin(false); setKinRelationship(""); setSocialProfiles([{key:0,platform:"",handle:""}]); nextSocialKey.current=1; setShowDocumentNotice(true); form.reset(); onCreated?.();
  ...
  return <DraftContext.Provider value={{values:draft,setField}}><div>
    ...
    {open && <div className="fixed inset-0 z-[120] bg-slate-950/45 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label="Register staff member"><div className="mx-auto flex h-full max-w-6xl flex-col overflow-hidden rounded-[1.75rem] bg-white shadow-2xl">
      ...
      </div><footer className="flex items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4 sm:px-7"><button type="button" disabled={step===0||busy} onClick={event=>changeStep(step-1,event.currentTarget.form)} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-black disabled:opacity-40">Previous</button><div className="flex gap-2"><button type="button" onClick={()=>setOpen(false)} className="rounded-xl bg-red-700 px-4 py-2.5 text-sm font-black text-white hover:bg-red-800">Cancel</button>{step<steps.length-1?<button type="button" onClick={event=>continueRegistration(event.currentTarget.form)} className="tenant-primary-bg rounded-xl px-5 py-2.5 text-sm font-black text-white">Continue</button>:<button type="button" disabled={busy} onClick={event=>{submitRequested.current=true;event.currentTarget.form?.requestSubmit()}} className="tenant-primary-bg rounded-xl px-5 py-2.5 text-sm font-black text-white disabled:opacity-50">{busy?"Creating staff record…":"Create staff record"}</button>}</div></footer></form>
    </div></div>}
