...
export function ProfileDetail({ id, kind, endpoint, backPath, title, fields, sensitiveEndpoint }: Props) {
  ...
  async function uploadDocument(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!canManage || (kind !== "StaffProfile" && kind !== "Guardian")) return;
    const form = event.currentTarget;
    const values = new FormData(form);
    const file = values.get("file");
    if (!(file instanceof File) || !file.size || file.size > (kind === "Guardian" ? 2 : 10) * 1024 * 1024 ||
        !(kind === "Guardian" ? ["image/jpeg", "image/png"] : ["application/pdf", "image/jpeg", "image/png"]).includes(file.type)) {
      setError(kind === "Guardian" ? "Choose a JPEG or PNG image no larger than 2 MB." : "Choose a PDF, JPEG or PNG document no larger than 10 MB.");
      return;
    }
    setUploading(true); setError("");
    try {
      const begin = await fetch(`/api/backend/documents/${kind}/${id}/uploads`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: file.name, contentType: file.type, sizeBytes: file.size, category: values.get("category") }) });
      if (!begin.ok) throw new Error("The document upload could not be started.");
      const upload = await begin.json() as { fileId: string };
      const stored = await fetch(`/api/backend/documents/${upload.fileId}/content`, { method: "PUT", headers: { "Content-Type": file.type }, body: file });
...
<section id="documents" className="scroll-mt-6 rounded-3xl bg-white p-6 shadow-sm"><h2 className="text-xl font-black">Documents</h2>{(kind === "StaffProfile" || kind === "Guardian") && canManage && <form onSubmit={uploadDocument} className="mt-4 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-[1fr_2fr_auto]"><label className="text-xs font-bold text-slate-700">Category<select name="category" required className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">{kind === "Guardian" ? <><option value="photo">Photograph</option><option value="signature">Signature</option></> : <><option value="identity">Identity</option><option value="qualification">Qualification</option><option value="contract">Contract</option><option value="photo">Photograph</option><option value="other">Other</option></>}</select></label><label className="text-xs font-bold text-slate-700">{kind === "Guardian" ? "JPEG or PNG · up to 2 MB" : "PDF, JPEG or PNG · up to 10 MB"}<input name="file" type="file" accept={kind === "Guardian" ? "image/jpeg,image/png" : "application/pdf,image/jpeg,image/png"} required className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm"/></label><button disabled={uploading} className="self-end rounded-lg bg-emerald-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{uploading ? "Uploading…" : "Upload document"}</button></form>}{documents.length === 0 ? <p className="mt-3 text-slate-600">No accessible documents.</p> : <ul className="mt-4 space-y-3">{documents.map(document => <li key={String(document.id)} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 p-4"><span><strong>{String(document.fileName)}</strong><br /><small>{String(document.category)} · {String(document.status)}</small></span><span className="flex gap-2"><button onClick={() => void download(String(document.id))} className="rounded-lg border px-3 py-2 text-sm">Download</button>{canManage && <button onClick={() => void remove(String(document.id))} className="rounded-lg border border-red-300 px-3 py-2 text-sm text-red-700">Delete</button>}</span></li>)}</ul>}</section>
    </div>
  </main>;
}
