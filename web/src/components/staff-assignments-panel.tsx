...
type ClassSection = Option & { isActive: boolean; academicYearId: string };
type ClassSubject = { classSectionId: string; subjectId: string; subjectName: string };
type Structure = { academicYears: { id: string; status: number | string }[]; classSections: ClassSection[]; classSubjects: ClassSubject[] };
...
export function StaffAssignmentsPanel({ staffId, view = "all", onChanged }: { staffId: string; view?: "all" | "position" | "assignments"; onChanged?: () => void }) {
  ...
  const [structure, setStructure] = useState<Structure>({ academicYears: [], classSections: [], classSubjects: [] });
  ...
  async function addAssignment(event: FormEvent<HTMLFormElement>) {
    ...
    const subjectId = assignmentRole === "0" ? String(data.get("subjectId") ?? "") : null;
    if (assignments.some(item => item.classSectionId === classSectionId && item.role === Number(assignmentRole) && item.subjectId === subjectId)) {
      notify({ tone: "error", title: "Already assigned", message: "This staff member already has this class or subject assignment." });
      return;
    }
    setBusy(true);
    const response = await fetch("/api/backend/hr/teaching-assignments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ staffId, classSectionId, subjectId, role: Number(assignmentRole) }) });
    setBusy(false);
  ...
  const activeYearId = structure.academicYears.find(year => year.status === 1 || year.status === "Active")?.id;
  const currentSections = structure.classSections.filter(item => item.isActive && item.academicYearId === activeYearId);
  ...
  return <div className="space-y-6">
    ...
    {(view === "all" || view === "assignments") && <section id="staff-assignments" className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><h2 className="text-xl font-black text-slate-950">Class and subject assignments</h2><p className="mt-1 text-sm text-slate-500">Teaching responsibilities assigned to this staff member. Other operational duties are not yet managed here.</p>
      {canManage && staffPositions.some(item => item.category === 0) && <form onSubmit={addAssignment} className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><label className="text-xs font-bold text-slate-700">Class section<select required value={classSectionId} onChange={event => setClassSectionId(event.target.value)} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm"><option value="">Select class</option>{currentSections.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-xs font-bold text-slate-700">Responsibility<select value={assignmentRole} onChange={event => setAssignmentRole(event.target.value)} className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm"><option value="0">Subject teacher</option><option value="1">Class teacher</option><option value="2">Form teacher</option></select></label>{assignmentRole === "0" && <label className="text-xs font-bold text-slate-700">Subject<select name="subjectId" required className="mt-1 block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm"><option value="">Select subject</option>{subjects.map(item => <option key={item.subjectId} value={item.subjectId}>{item.subjectName}</option>)}</select></label>}<button disabled={busy || !activeYearId} className="tenant-primary-bg self-end rounded-xl px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">Add assignment</button></form>}
